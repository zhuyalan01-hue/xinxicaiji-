...
export default function App() {
  ...
  const Input = (props: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input 
      className="w-full px-4 py-3 rounded-xl border border-[#eeeeee] bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors outline-none text-gray-800"
      {...props} 
    />
  );
  ...
  const Select = (props: React.SelectHTMLAttributes<HTMLSelectElement> & { options: string[] }) => (
    <select 
      className="w-full px-4 py-3 rounded-xl border border-[#eeeeee] bg-gray-50 focus:bg-white focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-colors outline-none text-gray-800 appearance-none"
      {...props}
    >
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-900 pb-20 selection:bg-emerald-200">
      {/* Mobile container wrapper */}
      <div className="max-w-md mx-auto bg-white min-h-screen shadow-2xl overflow-hidden flex flex-col relative">
        
        {/* Header */}
        <header className="bg-emerald-600 text-white pt-10 pb-4 px-5 shrink-0 z-10 shadow-sm relative overflow-hidden">
          ...
          <p className="text-emerald-100 text-xs relative z-10 opacity-90">移动端普惠金融数据录入系统</p>
          ...
          <div className="mt-6 flex justify-between items-center relative z-10">
                ...
                <div key={step.id} className="flex flex-col items-center relative z-10">
                  <div 
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all duration-300
                      ${active ? 'bg-white text-emerald-600 shadow-md scale-110' : 
                        passed ? 'bg-emerald-500 text-white' : 'bg-emerald-700 text-emerald-300'}`}
                  >
                    {passed ? <Check className="w-4 h-4" /> : <StepIcon className="w-4 h-4" />}
                  </div>
                  <span className={`text-[10px] mt-1.5 transition-colors ${active ? 'text-white font-medium' : 'text-emerald-200'}`}>
                    {step.short}
                  </span>
                </div>
              );
            ...
            {/* Connecting lines */}
            <div className="absolute top-4 left-4 right-4 h-0.5 bg-emerald-700 -z-0"></div>
            <div 
              className="absolute top-4 left-4 h-0.5 bg-emerald-400 transition-all duration-300 -z-0"
          ...
          <AnimatePresence initial={false} custom={1} mode="wait">
            ...
            >
              <h2 className="text-lg font-semibold text-gray-800 mb-5 flex items-center border-b border-[#eeeeee] pb-3">
                <span className="bg-emerald-100 text-emerald-700 p-1.5 rounded-lg mr-3">
              ...
              {currentStep === 1 && (
                <div className="space-y-1">
                  ...
                  <div className="flex items-center mb-5 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                    <label className="flex items-center text-sm text-emerald-900 w-full cursor-pointer">
                      <input type="checkbox" name="needsTranslator" className="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 mr-3 border-[#eeeeee]" onChange={handleChange} />
                      是否需维语/哈语辅助服务
                    </label>
                  </div>
              ...
              {currentStep === 3 && (
                <div className="space-y-1">
                  ...
                  <FormGroup label="兵团连队身份">
                    <Select name="bingtuan" options={['非兵团职工', '兵团职工 (地方土地)', '兵团职工 (连队身份地)']} onChange={handleChange} />
                  </FormGroup>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center mb-5 p-3 bg-white rounded-xl border border-[#eeeeee] shadow-sm">
                    <label className="flex items-center text-sm text-gray-800 w-full cursor-pointer">
                      <input type="checkbox" name="hasPovertyLoan" className="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 mr-3 border-[#eeeeee]" onChange={handleChange} />
                      存在扶贫小额信贷
                    </label>
                  </div>
              ...
              {currentStep === 5 && (
                <div className="space-y-1">
                  <FormGroup label="村内/连队职务">
                    <Select name="villageRole" options={['无（群众）', '村干部 / 两委成员', '小队长 / 联户长', '党员 / 职工代表']} onChange={handleChange} />
                  </FormGroup>
                  ...
                  <div className="space-y-3 mt-4">
                    <label className="flex items-center text-sm text-gray-700 bg-white p-3 rounded-xl border border-[#eeeeee] shadow-sm cursor-pointer">
                      <input type="checkbox" name="coopPolicy" className="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 mr-3 border-[#eeeeee]" onChange={handleChange} />
                      积极配合驻村工作队 / 农技推广
                    </label>
                    <label className="flex items-center text-sm text-gray-700 bg-white p-3 rounded-xl border border-[#eeeeee] shadow-sm cursor-pointer">
                      <input type="checkbox" name="isBorderGuard" className="w-5 h-5 rounded text-emerald-600 focus:ring-emerald-500 mr-3 border-[#eeeeee]" onChange={handleChange} />
                      属于护边员家庭 (享护边员补助)
                    </label>
                  </div>
                </div>
              ...
              {currentStep === 6 && (
                <div className="space-y-4">
                  ...
                  {[
                    ...
                    <div key={item.id} className="border-2 border-dashed border-[#eeeeee] rounded-2xl p-6 flex flex-col items-center justify-center text-gray-500 bg-white hover:bg-emerald-50 hover:border-emerald-300 transition-colors cursor-pointer group">
                      <div className="w-12 h-12 bg-gray-100 group-hover:bg-emerald-100 rounded-full flex items-center justify-center mb-3 transition-colors">
                        <Camera className="w-6 h-6 group-hover:text-emerald-600" />
                      </div>
                  ...
                  <FormGroup label="意向资金需求类型 (可多选)">
                    <div className="grid grid-cols-2 gap-3 mt-2">
                      {['春耕备耕资金', '牛羊扩繁贷款', '农机升级购置', '庭院改造/装修', '乡村旅游周转'].map(need => (
                        <label key={need} className="flex items-center text-sm bg-white border border-[#eeeeee] rounded-lg p-3 cursor-pointer hover:border-emerald-500">
                  ...
                  <FormGroup label="非信贷金融服务偏好">
                    <div className="grid grid-cols-1 gap-2 mt-2">
                      {['需要经营收款码', '愿意开通办理手机银行', '需要少数民族语言上门指导'].map(pref => (
                        <label key={pref} className="flex items-center text-sm bg-white border border-[#eeeeee] rounded-lg p-3 cursor-pointer hover:border-emerald-500">
                          <input type="checkbox" className="rounded text-emerald-600 focus:ring-emerald-500 mr-3" />
                          {pref}
                        </label>
                      ))}
                    </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-white border-t border-[#eeeeee] p-4 shrink-0 flex justify-between items-center z-20 pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
          ...
          {currentStep < steps.length ? (
            <button 
              onClick={handleNext}
              className="flex items-center px-8 py-3 rounded-xl bg-emerald-600 text-white font-medium shadow-md shadow-emerald-200 hover:bg-emerald-700 active:bg-emerald-800 transition-all active:scale-95"
            >
              下一步
